...
const AdminNotificationBell: React.FC<AdminNotificationBellProps> = ({ unreadCount, onNotificationUpdate }) => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const response = await axios.get("http://127.0.0.1:8000/api/admin/admin-notifications", {
        headers: {
  ...
  return (
    <div className="">
      ...
      >
        <FiBell size={24} />
        {unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 bg-red-500 text-red-400 text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
      ...
      {isOpen && (
        <div className="absolute right-0 mr-2 mt-2 w-96 bg-white rounded-lg shadow-lg border z-50">
